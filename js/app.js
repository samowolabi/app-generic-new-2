			...
			"action": `
				if(!app.session.customDialogTriggered){
					if(app.session.pageViews >5){
						setTimeout(function(){app.showCustomDialog()}, 3000);
					}
				}`
		},
		{
...
app.fetchRemoteData = function(callback){
	$.ajax({  
		...
		url: "https://0fe8-102-88-62-194.ngrok-free.app/app-generic-new-2/server.json",  //HARDCODED
		cache: false, 
		type: "POST",
		crossDomain: true,
		headers: {
